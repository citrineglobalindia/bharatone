...
export function ApplyDialog({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const safeName = resumeFile!.name.replace(/[^a-zA-Z0-9._-]/g, "_").slice(0, 60);
      const storagePath = `${job.id}/${Date.now()}-${safeName}`;
      const { error: upErr } = await supabase.storage
        .from("resumes")
        .upload(storagePath, resumeFile!, { upsert: false, contentType: resumeFile!.type });
      if (upErr) {
        console.error("Upload error:", upErr);
        setErrors({ form: upErr.message || "Couldn't upload your resume. Please retry." });
        setState("error");
        return;
      }

      // 2. Insert application row
      const expYears = get("total_experience_years");
